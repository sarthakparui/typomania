...
//this.prototype.frontWallLimit = 0;
...
c3dl.Scene = function ()
{
  ...
  //this is my custom variable to mark the rear wall which will be used to simulate collision
  //for the (invisible) front wall. this will be pushed along the with the object with which
  //the collision of the front wall will be simulated. when the rear wall is added to the system
  //check will be done to see if the object is the rear wall and will be saved in this variable

  //var myFrontWall;
  //c3dl.Scene.prototype.myFrontWallZValue=-250;
  //c3dl.Scene.prototype.myRearWallZValue = 175;
  //var myLeftWallXValue = 120;
  //var myRightWallXValue = -190;
  //var myTopWallYValue = 160;
  //var myBottomWallYValue = -95;
  //c3dl.Scene.prototype.myLeftWallXValue = 120;
  //c3dl.Scene.prototype.myRightWallXValue = -190;
  //c3dl.Scene.prototype.myTopWallYValue = 160;
  //c3dl.Scene.prototype.myBottomWallYValue = -95;
 ...
 /**
  ...
  this.addObjectToScene = function (obj)
  {
    ...
    case c3dl.SHAPE:
      ...
      //Listening for when rear wall is added (rear wall has the ownCollisionIndex property
      //as -5. Whenever it is added, we create a new collada object with ownCollisionIndex
      //as -6 to indicate front wall. this object will only be pushed into the collisionList
      //but never rendered on scene, so the object wont be present in the objList array
      
      /*if(obj.ownCollisionIndex == -5)
      {
          myFrontWall = new c3dl.Collada();
          myFrontWall.init("../IMGS/plane3D.dae");
          myFrontWall.ownCollisionIndex = -6;
          myFrontWall.isBall = 0;
          myFrontWall.lastCollisionIndex = -8;
          //objList.push(myFrontWall);
      }*/
      if(obj.ownCollisionIndex == -6)
      {
          frontWallIndex = objList.length - 1;
      }
   ...
   collisionList=[];
    ...
    for (var i = 0, len = objList.length; i < len; i++)
    {
      ...
      // update callback they write.
   	  var isCollidingWithWall = 0;
          if(objList[i].isBall == 0)
          {
              objList[i].update(timeElapsed);
              continue; // no need to go into further calculations for walls
          }
       /*************************Sougata's mod*******************************/
      switch (objList[i].getObjectType()) {
        ...
        case c3dl.SHAPE:
          ...
          //Collision
          /**********Sougata's mod**********/
          //simulating collision with front face of the box (which does not exist!! :P)
          //edit: now simulating all wall collisions from here, so wall objects are not pushed into collision list
          //note: we have set custom collada property isBall to denote whether an object is a wall or a ball
          
          var pos = objList[i].getPosition();
          var xPos = pos[0];
          var yPos = pos[1];
          var zPos = pos[2];
          /*if(zPos <= myFrontWallZValue)
          {
              //simulate collision
              collisionList.push(objList[i]);
              //push the rear wall to simulate the collision with front wall (wtf!!)
              //collisionList.push(myFrontWall);

              //var tmp = new c3dl.Collada();
              //tmp.init("../IMGS/point.dae");
              //tmp.ownCollisionIndex = -6;
              //tmp.isBall = 0;
              //tmp.lastCollisionIndex = -8;

              //var tmp = new c3dl.Collada();
              //tmp = objList[i];
              //tmp.ownCollisionIndex = -5;
              //tmp.lastCollisionIndex = -8;
              //tmp.isBall = 0;

              //collisionList.push(tmp);

              collisionList.push(objList[frontWallIndex]);


          }*/
			  if(xPos >= this.myLeftWallXValue || xPos <= this.myRightWallXValue || yPos >= this.myTopWallYValue || yPos <= this.myBottomWallYValue || zPos >= this.myRearWallZValue || zPos <= this.myFrontWallZValue)
			  {
			  	var b = objList[i].getLinearVel();
			  	if(xPos <= this.myRightWallXValue || xPos >= this.myLeftWallXValue)
				  {
					b[0] = -b[0];
					if(xPos <= this.myRightWallXValue)
					{
						//objList[i].setPosition([-194,yPos,zPos]);
						xPos = this.myRightWallXValue + 5;
					}
					else
					{
						//objList[i].setPosition([124,yPos,zPos]);
						xPos = this.myLeftWallXValue - 5;
                                                //alert(this.myLeftWallXValue);
					}
				  }
			  	if(yPos <= this.myBottomWallYValue || yPos >= this.myTopWallYValue)
				  {
					//alert(this.myTopWallYValue + ","+ this.myBottomWallYValue);
                                        b[1] = -b[1];
					if(yPos <= this.myBottomWallYValue)
					{
						//objList[i].setPosition([xPos,-99,zPos]);
						yPos = this.myBottomWallYValue + 5;
					}
					else
					{
						//objList[i].setPosition([xPos,164,zPos]);
						yPos = this.myTopWallYValue - 5;
					}
				  }
			  	if(zPos >= this.myRearWallZValue || zPos <= this.myFrontWallZValue)
                                //if(zPos >= 175 || zPos <= -115)
				  {
					
                                        b[2] = -b[2];
					if(zPos >= this.myRearWallZValue)
                                        //if(zPos >= 175)
					{
						//objList[i].setPosition([xPos,yPos,myFrontWallZValue + 1]);
						zPos = this.myRearWallZValue - 5;
                                                //zPos = 170;
					}
					else
					{
						//objList[i].setPosition([xPos,yPos,179]);
						zPos = this.myFrontWallZValue + 5;
                                                //zPos = -110;
					}
				  }
				  objList[i].setLinearVel([b[0], b[1], b[2]]);
				  objList[i].setPosition([xPos, yPos, zPos]);
				  isCollidingWithWall = 1;//ball is colliding with wall, so no need to check its collision with other balls right now
			  }

          if (collision && isCollidingWithWall == 0) {
...
//@private custom implemented method to delete all the balls in the scene at end of game
    c3dl.Scene.prototype.removeAllBalls = function()
    {
        for(var i = 0; i < objList.length; ++i)
            {
                if(objList[i].isBall == 1)
                    this.removeObjectFromScene(objList[i]);
            }
    }
